...
existing tab and API — including Link Security and /api/check-url — keeps
running from its own files, unchanged. new.py only ADDS:

  • User Portal                 /api/portal/meta, /api/portal/score
  • Live Prediction fix         scaled input + calibrated tiers for /api/predict
  • Investigation Workbench     /api/wb/*  (alert workbench, transaction lookup,
                                account search + history, explainable risk card,
                                linked-account network, analyst workflow + audit)
...
import app as base   # the existing FinShield AI backend (all tabs + Link Security)
...
def api_wb_explain(row):
    ...
    if not TXN.model_ready:
        return jsonify(error='cleaned_transactions.csv lacks model columns: ' +
                             ', '.join(TXN.model_missing)), 409
    r = TXN.fetch([row])[0]
    feats = _model_row(r)
    ...
    try:
        ...
        return jsonify(error=f'SHAP computation failed: {ex}'), 500
...
def api_wb_account(ref):
    ...
    summary = dict(
        ...
        first_seen=dict(day=int(tk.min() // 24), hour=int(tk.min() % 24)),
        last_seen=dict(day=int(tk.max() // 24), hour=int(tk.max() % 24)),
